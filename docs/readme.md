...
Lantah is a decentralized, federated peer-to-peer network that allows people to
send payments in any asset anywhere in the world instantaneously, and with
minimal fee.

`GramR` is the core component of this network. `GramR` is a C++
...
- [GramR administration](software/admin.md)
...
* **Herder** is responsible for interfacing between SCP and the rest of
  `gramr`. Herder provides SCP with concrete implementations of the
